...
let player = {
//
    name: rs.question(`What is your name?`),
    lifePower: 100,
    inventory: []
}
let enemies = [
...
function choseWalk() {
    ...
    if (escape < 3) {
       ...
       // console.log(`However, ${enemies.name} wanted a second chance and returned to inflict some damage. Your Life Power is now ${player.lifePower}`)
        ...
        let fightOrRun = rs.question(`Would you like to fight ${enemies.name} or run? Please enter "f" to fight or any other key to run. `);
        if (fightOrRun === 'f') {
            choseFight()
        } else {
            choseRun()
